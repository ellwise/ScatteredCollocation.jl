...
hex = false
geoshape = hex ? Triangle : Quadrilateral
#refshape = hex ? RefTriangle    : RefSquare
order = hex ? 2 : 1;

numdims = 2
corner1 = Vec{numdims}((0.0, 0.0))
corner2 = Vec{numdims}((10.0, 1.0))
grid = generate_grid(geoshape, (60, 6), corner1, corner2)
...
for row in nodesets[:left]
    ...
    R = ScatteredCollocation.differentiate(p0, p[nn], [0, 0])
...
for row in nodesets[:interior]
    ...
    DxDy = ScatteredCollocation.differentiate(p0, p[nn], [1, 1])
    DyDx = DxDy
    DyDy = ScatteredCollocation.differentiate(p0, p[nn], [0, 2])
    # A = (λ+μ).A1 + μ.A2
    L[row+0*num_points, nn.+0*num_points] = (λ+μ).*DxDx + μ.*(DxDx+DyDy)
    L[row+0*num_points, nn.+1*num_points] = (λ+μ).*DxDy
    L[row+1*num_points, nn.+0*num_points] = (λ+μ).*DyDx
    L[row+1*num_points, nn.+1*num_points] = (λ+μ).*DyDy + μ.*(DxDx+DyDy)
    R = ScatteredCollocation.differentiate(p0, p[nn], [0, 0])
...
plotly()
scatter(x+ux, y+uy, markersize=2)
